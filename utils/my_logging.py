...
def initialize_best_metrics(goal="minimize"):
    assert goal in [
        "minimize",
        "maximize",
    ], "Invalid goal: must be either 'minimize' or 'maximize'"
    return {
...
def update_best_metrics(best_metrics, val_metric, test_metric, epoch, goal="minimize"):
    ...
    if (goal == "minimize" and val_metric < best_metrics["val_loss"]) or (
        ...
        best_metrics.update(
            {"val_loss": val_metric, "test_loss": test_metric, "epoch": epoch}
        )
    return best_metrics
...
def set_posfix(optim, loss_list, val_metric, test_metric, pbar):
    try:
        ...
        lr = -1
    pbar.set_postfix(
        {"lr": lr, "loss": np.mean(loss_list), "val": val_metric, "test": test_metric}
    )
