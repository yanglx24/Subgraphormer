...
def get_dataloader(cfg, sample_idx=0):
    ...
    if "zinc" in cfg.data.name:
        if "12k" in cfg.data.name:
            dataloader, num_elements_in_target = get_zinc12k_dataloader(
                cfg=cfg, transform_suffix=transform_suffix, transforms=transforms
            )
        elif "full" in cfg.data.name:
            dataloader, num_elements_in_target = get_zincfull_dataloader(
                cfg=cfg, transform_suffix=transform_suffix, transforms=transforms
            )
        else:
        ...
        cfg.data.name == "ogbg-moltox21"
        or cfg.data.name == "ogbg-molesol"
        or cfg.data.name == "ogbg-moltoxcast"
        or cfg.data.name == "ogbg-molbace"
        or cfg.data.name == "ogbg-molhiv"
    ):
        dataloader, num_elements_in_target = get_ogb_dataloader(
            cfg=cfg, transform_suffix=transform_suffix, transforms=transforms
        )
    elif cfg.data.name + "_full" == "alchemy_full":
        dataloader, num_elements_in_target = get_alchemy_dataloader(
            cfg=cfg, transform_suffix=transform_suffix, transforms=transforms
        )
    elif cfg.data.name == "Peptides-func":
        dataloader, num_elements_in_target = get_peptides_func_dataloader(
            cfg=cfg, transform_suffix=transform_suffix, transforms=transforms
        )
...
def get_ogb_dataloader(cfg, transform_suffix, transforms):
    dataset = PygGraphPropPredDataset(
        name=cfg.data.name, root=cfg.data.dir + transform_suffix, transform=transforms
    )
    dataloader = {
