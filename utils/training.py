...
def get_loss_func(cfg):
    ...
    if dataset_name not in dataset_info:
        raise ValueError(f"No loss function available for the dataset: {dataset_name}")
...
def get_optim_func(cfg, model):
    ...
    optimizers = {
        ...
        "Peptides-func": torch.optim.AdamW(
            model.parameters(),
            lr=lr,
            betas=(0.9, 0.999),
            eps=1e-8,
    ...
    if dataset_name not in optimizers:
        raise ValueError(f"No Optimizer available for the dataset: {dataset_name}")
...
def get_sched_func(cfg, optim, warmup_epochs=10):
    ...
    sched = {
        "zinc12k": torch.optim.lr_scheduler.ReduceLROnPlateau(
            optim, mode="min", factor=0.5, patience=cfg.training.patience, verbose=True
        ),
        "zincfull": torch.optim.lr_scheduler.ReduceLROnPlateau(
            optim, mode="min", factor=0.5, patience=cfg.training.patience, verbose=True
        ),
        "ogbg-molesol": torch.optim.lr_scheduler.ConstantLR(
            optim, total_iters=0, factor=1
        ),
        "ogbg-molbace": torch.optim.lr_scheduler.ConstantLR(
            optim, total_iters=0, factor=1
        ),
        "alchemy": torch.optim.lr_scheduler.ReduceLROnPlateau(
            optim,
            mode="min",
            factor=0.5,
    ...
    if dataset_name not in sched:
        raise ValueError(f"No Scheduler available for the dataset: {dataset_name}")
...
class WarmupCosineAnnealingLR(_LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_epochs:
            lr = (
                self.initial_lr
                + (self.max_lr - self.initial_lr) * self.last_epoch / self.warmup_epochs
            )
        else:
            cosine_epoch = torch.tensor(float(self.last_epoch - self.warmup_epochs))
            cosine_epochs = torch.tensor(float(self.total_epochs - self.warmup_epochs))
            lr = (
                self.max_lr
                * (1 + torch.cos(torch.pi * cosine_epoch / cosine_epochs))
                / 2
            )
            lr = lr.item()
...
class ZincLEvaluator(nn.L1Loss):
    ...
    def eval(self, input_dict):
        ...
        L1_val_dict = {"L1loss": L1_val.item()}
        return L1_val_dict
...
def train_loop_ASAM(model, loader, critn, optim, epoch, device, task="regression"):
    ...
    for i, batch in enumerate(pbar):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            y = (
                batch.y.view(pred.shape).to(torch.float32)
                if pred.size(-1) == 1
                else batch.y
            )
            ...
            pbar.set_description(f"Epoch {epoch} Train Step {i}: Loss = {loss.item()}")
...
def eval_loop_peptides(
    model, loaders, eval, device, average_over=1, task="regression", eval_type="val"
):
    assert eval_type in [
        "val",
        "test",
    ], f"The variable eval_type has to be either 'val' or 'test'"
    model.eval()
    # Warning: for average_over > 0 this works only if test/val dataloader doesn't shuffle!!!
    input_dict_for_votes = []
    for vote in range(average_over):
