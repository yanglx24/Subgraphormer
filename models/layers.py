...
class Atom(nn.Module):
    ...
    def __init__(
        ...
        if use_linear:
            ...
            logging.info(
                "Using a look up table to encode atoms -- not uses atom_dim variable."
            )
        self.use_linear = use_linear
        self.embed_v = encode and AtomEncoder(dim)

        if use_linear:  # linear layer
            self.embed_v = encode and nn.Sequential(
                nn.Linear(atom_dim, dim), nn.ReLU(), nn.Linear(dim, dim)
            )
        else:  # look up table
            self.embed_v = encode and AtomEncoder(dim)
...
class Bond(nn.Module):
    ...
    def __init__(self, dim: int, linear: bool = False, linear_in_dim: int = 4):
        super().__init__()
        self.linear = linear
        if self.linear:
            logging.info("Using an MLP to encode bonds -- uses atom_dim variable.")
            self.bond_encoder = nn.Sequential(
                nn.Linear(linear_in_dim, dim), nn.ReLU(), nn.Linear(dim, dim)
            )
        else:
            logging.info(
                "Using a look up table to encode bonds -- not uses edge_attr_dim variable."
            )

        self.embed = BondEncoder(dim)

    def forward(
        self, message, attrs, dont_use_message=False, peptides_flag=False, SSWL=False
    ):
        if attrs is None:
        ...
        if dont_use_message:
            ...
            if attr_of_each_edge.shape[-1] == 1:
                ...
                return F.relu(
                    self.embed(attr_of_each_edge.to(torch.int)[:, None]).mean(dim=1)
                )  # Graphs x Embs
...
class Attention_block(torch.nn.Module):
    def __init__(self, d, H=1, d_output=64, edge_dim=64, type="Gat"):
        ...
        assert self.d_output == (
            (self.d_output // self.H) * self.H
        ), f"Invalid self.d_output value. Expected: {(self.d_output // self.H) * self.H}, but got: {self.d_output}."
        if type == "GatV2":
            self.attn_layer = gnn.GATv2Conv(
                in_channels=self.d,
                out_channels=self.d_output // self.H,
                heads=self.H,
